...
- 다음 공모전 핵심 개발 과제로 **안동 문화 코스**를 우선 설계하기로 결정. 단순 장소 나열이 아니라 박물관·문화공간·주변 관광지·이동 흐름을 연결한 대표 사용자 여정으로 구성하고, 공모전 시연 포인트에 맞춘다.
- 배지 비주얼은 현재 Black Gallery 방향을 유지하면서 더 세련된 컬렉터블 디자인으로 고도화한다. 대표 3종 시안 → 실기기 QA → 비주얼 시스템 확정 → 나머지 확장 순서를 유지한다.
- App Store/Google Play 소개 이미지는 현재 단순 화면 캡처형에서 벗어나, 큰 헤드라인·브랜드 배경·기능별 앱 목업을 조합한 **프로모션형 스토어 이미지 세트**로 향후 전면 개편하기로 결정. 첫 3~5장에서 Muselry 핵심 가치가 바로 이해되도록 설계한다.
