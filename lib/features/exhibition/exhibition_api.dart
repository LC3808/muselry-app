...
class ExhibitionApi {
  ...
  /// sido 기준 현재 진행 중인 전시 목록 반환 (캐시 우선)
  ...
  Future<List<Exhibition>?> fetchExhibitions(String sido) async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('EXH-API: pages loaded=1-3 전시 filter=${exhibitions.length}');
      }
  ...
  Future<List<Exhibition>?> fetchExhibitionsExtra(
    ...
    try {
      ...
      if (kDebugMode) {
        print(
            'EXH-API: pages loaded=1-5 전시 filter=${combined.length} (extra=${extraExhibitions.length})');
      }
